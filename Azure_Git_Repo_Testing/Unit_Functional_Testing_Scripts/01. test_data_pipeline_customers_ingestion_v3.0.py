...
# MAGIC #### Purpose:
# MAGIC To verify the end-to-end data ingestion process from various sources and data standardization for `customers` data, ensuring data quality and reliability throughout the pipeline.
# MAGIC
...
def test_data_extraction_process():
    ...
    try:
        ...
        try:
            ...
            if files:
                try:
                    test_file = "olist_customers_dataset.csv"
...
# MAGIC    ```
# MAGIC    ✓ Pipeline started. Run ID: 76d40274-d5cc-11ef-aa94-00163e79c74e
...
ADF_CONFIG = {
    ...
    'http_source': 'https://raw.githubusercontent.com/YvonneLipLim/JDE05_Final_Project/refs/heads/main/Datasets/Olist/olist_customers_dataset.csv', # Change the http source path if needed
    'subscription_id': '781d95ce-d9e9-4813-b5a8-4a7385755411',
    'key_vault_url': 'https://Olist-Key.vault.azure.net/',
    'scope': 'https://management.azure.com/.default',
    'destination_path': '/mnt/olist-store-data/raw-data/olist_customers_dataset.csv', # Change the destination path if needed
...
# MAGIC #### Purpose:
# MAGIC To validate the configuration of Synapse workspace components for `customers` data ingestion pipeline.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC   ```
# MAGIC   ✓ Pipeline Name:IngestCustomersDataToOlistDB
...
# MAGIC - Container: olist-store-data
# MAGIC - File Path: transformed-data/olist_customers_cleaned_dataset_v2.0.parquet
# MAGIC
...
# MAGIC
# MAGIC **_Conclusion_**:<br>
# MAGIC The test successfully demonstrated the ability to configure Synapse workspace components, establishing a robust infrastructure for `customers` data ingestion. The configuration provides a solid foundation for further data pipeline development and integration.
...
def comprehensive_synapse_data_flow_test():
    ...
    try:
        ...
        pipeline_operation = client.pipeline.begin_create_or_update_pipeline(
            pipeline_name="IngestCustomersDataToOlistDB",
        ...
        pipeline = client.pipeline.get_pipeline(pipeline_name="IngestCustomersDataToOlistDB")
...
# MAGIC #### Purpose:
# MAGIC To validate the access and data consistency between Synapse SQL views and external tables for `customers` data.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC   ```
# MAGIC   ✓ View Name: customers_view
# MAGIC   ✓ Row Count: 99,700
...
# MAGIC %md
# MAGIC ### Test 5: Data Cleaning Pipeline for Customers Dataset
# MAGIC #### Purpose:
# MAGIC To validate the comprehensive data cleaning and standardization pipeline that ensures `customers` location data meets quality standards and business requirements while maintaining data integrity.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC
# MAGIC **_Conclusion_**:<br>
# MAGIC The data cleaning pipeline successfully implements a robust and comprehensive approach to standardizing customer location data. The implementation demonstrates:
# MAGIC
...
from pyspark.sql.functions import (
    col, when, upper, initcap, length, count, desc, 
    sum, format_number, lower, udf, regexp_replace, lit,
    lpad as spark_lpad
)
...
# COMMAND ----------
# Test utilities
def create_test_customers_df():
    """Create a test DataFrame with sample customer data"""
    test_data = [
        ("1234", "CUST001", "12345", "sao paulo", "SP"),
        ("1235", "CUST002", "23456", "bh", "MG"),
        ("1236", "CUST003", "34567", "RiO dE jAnEiRo", "RJ"),
        ("1237", "CUST004", "1234", "quilometro 14", "MG"),
        ("1238", "CUST005", "56789", "sao paulo das flores", "SP")
    ]
...
def create_test_geolocation_df():
    ...
    test_data = [
        ("12345", "Sao Paulo", "SP"),
        ("23456", "Belo Horizonte", "MG"),
        ("34567", "Rio de Janeiro", "RJ")
    ]
    
    schema = StructType([
        StructField("geolocation_zip_code_prefix", StringType(), True),
        StructField("geolocation_city_final", StringType(), True),
...
def test_zip_code_padding():
    ...
    try:
        customers_df = create_test_customers_df()
        
        padded_df = customers_df.withColumn(
            "customer_zip_code_prefix",
            spark_lpad(col("customer_zip_code_prefix").cast("string"), 5, "0")
        )
        
        short_zips = padded_df.filter(length("customer_zip_code_prefix") < 5).count()
        assert short_zips == 0, "Found ZIP codes shorter than 5 digits"
        
        test_zip = padded_df.filter(col("customer_id") == "1237").select("customer_zip_code_prefix").collect()[0][0]
        assert test_zip == "01234", f"Expected 01234, got {test_zip}"
    ...
    try:
        ...
        def clean_city_name(city):
            ...
            if 'quilometro 14' in city or 'km 14' in city:
                return 'Mutum'
            words = city.split()
            cleaned_words = [word.capitalize() if word not in ['de', 'da', 'do', 'das', 'dos', 'e'] or i == 0 
                           else word for i, word in enumerate(words)]
            return ' '.join(cleaned_words)
        
        clean_city_udf = udf(clean_city_name, StringType())
        cleaned_df = customers_df.withColumn("customer_city", clean_city_udf(col("customer_city")))
        
        assert cleaned_df.filter(col("customer_city").isNull()).count() == 0, "Found null city names"
        
        test_cases = {
            "bh": "Belo Horizonte",
            "quilometro 14": "Mutum",
            "sao paulo das flores": "Sao Paulo das Flores"
        }
        
        for original, expected in test_cases.items():
            actual = cleaned_df.filter(lower(col("customer_city")) == original.lower()) \
                              .select("customer_city").collect()
            if actual:
                assert actual[0][0] == expected, f"Expected {expected}, got {actual[0][0]}"
...
def test_geolocation_merging():
    """Test merging with geolocation data"""
    try:
        customers_df = create_test_customers_df()
        geolocation_df = create_test_geolocation_df()
        
        geolocation_cities = geolocation_df.select(
            col("geolocation_zip_code_prefix"),
            col("geolocation_city_final").alias("geolocation_city")
        ).distinct()
        
        merged_df = customers_df.join(
            geolocation_cities,
            customers_df.customer_zip_code_prefix == geolocation_cities.geolocation_zip_code_prefix,
            "left"
        )
        ...
        assert final_df.count() == customers_df.count(), "Row count changed after merging"
        assert final_df.filter(col("customer_city").isNull()).count() == 0, "Found null cities after merging"
        
        print("✓ Geolocation merging test passed")
    except Exception as e:
        print(f"❌ Geolocation merging test failed: {str(e)}")
        raise
