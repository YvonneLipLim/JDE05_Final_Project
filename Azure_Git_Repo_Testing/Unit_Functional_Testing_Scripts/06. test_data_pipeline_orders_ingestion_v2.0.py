...
# MAGIC #### Purpose:
# MAGIC To verify the end-to-end data ingestion process from various sources and data standardization for `orders` data, ensuring data quality and reliability throughout the pipeline.
# MAGIC
...
# MAGIC    ```
# MAGIC    ✓ Pipeline started. Run ID: 5f5bbf82-d5d7-11ef-b564-00163e79c74e
...
# MAGIC    ```
# MAGIC    ✓ Source data read: 99,441 rows
# MAGIC    ✓ Destination data read: 99,441 rows
# MAGIC    ✓ Data transfer verified. 99,441 rows transferred successfully
# MAGIC    ```
...
# MAGIC - Data transferred completely (99,441 rows)
...
# MAGIC    - Perfect data integrity maintenance with 99,441 rows accurately transferred
# MAGIC
...
ADF_CONFIG = {
    ...
    'http_source': 'https://raw.githubusercontent.com/YvonneLipLim/JDE05_Final_Project/refs/heads/main/Datasets/Olist/olist_orders_dataset.csv', # Change the http source path if needed
    'subscription_id': '781d95ce-d9e9-4813-b5a8-4a7385755411',
    'key_vault_url': 'https://Olist-Key.vault.azure.net/',
    'scope': 'https://management.azure.com/.default',
    'destination_path': '/mnt/olist-store-data/raw-data/olist_orders_dataset.csv', # Change the destination path if needed
...
# MAGIC #### Purpose:
# MAGIC To validate the configuration of Synapse workspace components for `orders` data ingestion pipeline.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC   ```
# MAGIC   ✓ Pipeline Name:IngestOrdersDataToOlistDB
...
# MAGIC - Container: olist-store-data
# MAGIC - File Path: transformed-data/olist_orders_cleaned_dataset_v2.0.parquet
# MAGIC
...
# MAGIC
# MAGIC **_Conclusion_**:<br>
# MAGIC The test successfully demonstrated the ability to configure Synapse workspace components, establishing a robust infrastructure for `orders` data ingestion. The configuration provides a solid foundation for further data pipeline development and integration.
...
def comprehensive_synapse_data_flow_test():
    ...
    try:
        ...
        pipeline_operation = client.pipeline.begin_create_or_update_pipeline(
            pipeline_name="IngestOrdersDataToOlistDB",
        ...
        pipeline = client.pipeline.get_pipeline(pipeline_name="IngestOrdersDataToOlistDB")
...
# MAGIC #### Purpose:
# MAGIC To validate the access and data consistency between Synapse SQL views and external tables for `orders` data.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC   ```
# MAGIC   ✓ View Name: orders_view
# MAGIC   ✓ Row Count: 99,441
...
# MAGIC %md
# MAGIC ### Test 5: Data Cleaning Pipeline for Orders Dataset
# MAGIC #### Purpose:
# MAGIC To validate the comprehensive data cleaning and standardization pipeline that ensures `orders` data meets quality standards and business requirements while maintaining data integrity.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC
# MAGIC **Conclusion**:<br>
# MAGIC The data cleaning pipeline successfully implements a robust and comprehensive approach to standardizing `orders` data. The implementation demonstrates:
# MAGIC
...
from pyspark.sql.functions import (
    col, when, to_timestamp, initcap, datediff,
    year, month, dayofweek, expr, isnan
)
from datetime import datetime, timedelta

# COMMAND ----------
# Execute test
def create_test_orders_df():
    """Create a test DataFrame with sample orders data"""
    test_data = [
        # Valid orders
        ("ORDER1", "CUST1", "delivered", "2018-01-01 10:00:00", "2018-01-01 14:00:00", 
         "2018-01-02 10:00:00", "2018-01-05 14:00:00", "2018-01-10 00:00:00"),
        
        # Missing delivery date but marked as delivered
        ("ORDER2", "CUST2", "delivered", "2018-01-02 09:00:00", "2018-01-02 11:00:00", 
         "2018-01-03 15:00:00", None, "2018-01-12 00:00:00"),
        
        # Delivered but marked as canceled
        ("ORDER3", "CUST3", "canceled", "2018-01-03 11:00:00", "2018-01-03 14:00:00", 
         "2018-01-04 10:00:00", "2018-01-06 15:00:00", "2018-01-15 00:00:00"),
        
        # Valid canceled order
        ("ORDER4", "CUST4", "canceled", "2018-01-04 16:00:00", None, None, None, 
         "2018-01-20 00:00:00"),
         
        # On-time delivery
        ("ORDER5", "CUST5", "delivered", "2018-01-05 10:00:00", "2018-01-05 12:00:00",
         "2018-01-06 09:00:00", "2018-01-08 14:00:00", "2018-01-10 00:00:00")
    ]
...
def test_timestamp_conversion():
    """Test timestamp conversion and validation"""
    try:
        orders_df = create_test_orders_df()
        ...
        sample_timestamp = orders_df.filter(col("order_id") == "ORDER1")\
                                  .select("order_purchase_timestamp").collect()[0][0]
        assert sample_timestamp.year == 2018, "Incorrect year in timestamp conversion"
        
        print("✓ Timestamp conversion test passed")
    except Exception as e:
        print(f"❌ Timestamp conversion test failed: {str(e)}")
        raise

def test_order_status_standardization():
    """Test order status cleaning and standardization"""
    try:
        orders_df = create_test_orders_df()
        
        # Standardize status
        cleaned_df = orders_df.withColumn(
            "order_status",
        ...
        # Verify status corrections
        # Check ORDER2: Should be changed to "Shipped" (Delivered without delivery date)
        order2_status = cleaned_df.filter(col("order_id") == "ORDER2")\
                                 .select("order_status").collect()[0][0]
        assert order2_status == "Shipped", f"Expected Shipped, got {order2_status}"
        
        # Check ORDER3: Should be changed to "Delivered" (Canceled but has delivery date)
        order3_status = cleaned_df.filter(col("order_id") == "ORDER3")\
                                 .select("order_status").collect()[0][0]
        assert order3_status == "Delivered", f"Expected Delivered, got {order3_status}"
        
        print("✓ Order status standardization test passed")
    except Exception as e:
        print(f"❌ Order status standardization test failed: {str(e)}")
        raise

def test_delivery_metrics():
    """Test delivery time calculations and performance metrics"""
    try:
        orders_df = create_test_orders_df()
        
        # Convert timestamps for calculations
        timestamp_columns = [
        ...
        metrics_df = orders_df.withColumn(
            "processing_time",
            when(
        ...
        # Verify metrics for ORDER1
        order1 = metrics_df.filter(col("order_id") == "ORDER1").collect()[0]
        assert order1.processing_time == 0, "Incorrect processing time calculation"
        assert order1.delivery_time == 4, "Incorrect delivery time calculation"
        assert order1.delivery_performance == "On Time", "Incorrect delivery performance"
        
        print("✓ Delivery metrics test passed")
    except Exception as e:
        print(f"❌ Delivery metrics test failed: {str(e)}")
        raise

def test_derived_features():
    """Test generation of derived features"""
    try:
        orders_df = create_test_orders_df()
        ...
        # Generate features
        features_df = orders_df.withColumn(
            "purchase_year",
        ...
        # Verify features for ORDER1
        order1 = features_df.filter(col("order_id") == "ORDER1").collect()[0]
        assert order1.purchase_year == 2018, "Incorrect purchase year"
        assert order1.purchase_month == 1, "Incorrect purchase month"
        assert order1.purchase_dayofweek in range(1, 8), "Invalid day of week"
        
        print("✓ Derived features test passed")
    except Exception as e:
        print(f"❌ Derived features test failed: {str(e)}")
        raise

# COMMAND ----------
# Run test
def run_all_tests():
    """Run all orders data cleaning tests"""
    print("Running all orders data cleaning tests...")
    try:
        test_timestamp_conversion()
        test_order_status_standardization()
