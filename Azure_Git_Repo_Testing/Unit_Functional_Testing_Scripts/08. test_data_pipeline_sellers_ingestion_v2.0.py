...
# MAGIC #### Purpose:
# MAGIC To verify the end-to-end data ingestion process from various sources and data standardization for `sellers` data, ensuring data quality and reliability throughout the pipeline.
# MAGIC
...
def test_data_extraction_process():
    ...
    try:
        ...
        try:
            ...
            if files:
                try:
                    test_file = "olist_sellers_dataset.csv"
...
# MAGIC    ```
# MAGIC    ✓ Pipeline started. Run ID: a29042b2-d66f-11ef-9ef3-00163ed61b6a
...
# MAGIC    ```
# MAGIC    ✓ Source data read: 3,095 rows
# MAGIC    ✓ Destination data read: 3,095 rows
# MAGIC    ✓ Data transfer verified. 3,095 rows transferred successfully
# MAGIC    ```
...
# MAGIC - Data transferred completely (3,095 rows)
...
# MAGIC    - Perfect data integrity maintenance with 3,095 rows accurately transferred
# MAGIC
...
ADF_CONFIG = {
    ...
    'http_source': 'https://raw.githubusercontent.com/YvonneLipLim/JDE05_Final_Project/refs/heads/main/Datasets/Olist/olist_sellers_dataset.csv', # Change the http source path if needed
    'subscription_id': '781d95ce-d9e9-4813-b5a8-4a7385755411',
    'key_vault_url': 'https://Olist-Key.vault.azure.net/',
    'scope': 'https://management.azure.com/.default',
    'destination_path': '/mnt/olist-store-data/raw-data/olist_sellers_dataset.csv', # Change the destination path if needed
...
# MAGIC #### Purpose:
# MAGIC To validate the configuration of Synapse workspace components for `sellers` data ingestion pipeline.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC   ```
# MAGIC   ✓ Pipeline Name:IngestSellersDataToOlistDB
...
# MAGIC - Container: olist-store-data
# MAGIC - File Path: transformed-data/olist_sellers_cleaned_dataset_v2.0.parquet
# MAGIC
...
# MAGIC
# MAGIC **_Conclusion_**:<br>
# MAGIC The test successfully demonstrated the ability to configure Synapse workspace components, establishing a robust infrastructure for `sellers` data ingestion. The configuration provides a solid foundation for further data pipeline development and integration.
...
def comprehensive_synapse_data_flow_test():
    ...
    try:
        ...
        pipeline_operation = client.pipeline.begin_create_or_update_pipeline(
            pipeline_name="IngestSellersDataToOlistDB",
        ...
        pipeline = client.pipeline.get_pipeline(pipeline_name="IngestSellersDataToOlistDB")
...
# MAGIC #### Purpose:
# MAGIC To validate the access and data consistency between Synapse SQL views and external tables for `sellers` data.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC   ```
# MAGIC   ✓ View Name: sellers_view
# MAGIC   ✓ Row Count: 3,095
...
CONFIG = {
    ...
    "database": "OlistSQLDB",
    "schema": "dbo",
    "view_name": "sellers_view",
    "external_table": "extsellers",
...
# MAGIC %md
# MAGIC ### Test 5: Data Cleaning Pipeline for Sellers Dataset
# MAGIC #### Purpose:
# MAGIC To validate the comprehensive data cleaning and standardization pipeline that ensures `sellers` data meets quality standards and business requirements while maintaining data integrity.
# MAGIC
# MAGIC #### Test Components and Results:
...
# MAGIC
# MAGIC **_Conclusion_**:<br>
# MAGIC The data cleaning pipeline successfully implements a robust and comprehensive approach to standardizing `sellers` data. The implementation demonstrates:
# MAGIC
...
from pyspark.sql.functions import (
    col, when, upper, initcap, length, count, trim,
    regexp_replace, split, lower, lpad
)

# COMMAND ----------
# Execute test
def create_test_sellers_df():
    """Create a test DataFrame with sample sellers data"""
    test_data = [
        # Valid records
        ("SELLER1", "12345", "são paulo", "SP"),
        ("SELLER2", "1234", "BH", "MG"),
        ("SELLER3", "34567", "RiO dE jAnEiRo/RJ", "RJ"),
        # Invalid records
        ("SELLER4", "89", "sp@", "SP"),
        ("SELLER5", "56789", "sao paulo-capital", "SP")
    ]
...
def test_zip_code_padding():
    """Test ZIP code padding functionality"""
    try:
        sellers_df = create_test_sellers_df()
        
        # Apply ZIP code padding
        padded_df = sellers_df.withColumn(
            "seller_zip_code_prefix",
            lpad(col("seller_zip_code_prefix").cast("string"), 5, "0")
        )
        
        # Verify results
        short_zips = padded_df.filter(length("seller_zip_code_prefix") < 5).count()
        assert short_zips == 0, "Found ZIP codes shorter than 5 digits"
        ...
        for seller_id, expected_zip in test_cases.items():
            actual_zip = padded_df.filter(col("seller_id") == seller_id)\
                                .select("seller_zip_code_prefix").collect()[0][0]
            assert actual_zip == expected_zip, f"For {seller_id}: Expected {expected_zip}, got {actual_zip}"
        
        print("✓ ZIP code padding test passed")
    except Exception as e:
        print(f"❌ ZIP code padding test failed: {str(e)}")
        raise
...
def test_city_name_cleaning():
    ...
    try:
        ...
        cleaned_df = sellers_df.withColumn(
            "seller_city",
        ...
        # Apply character standardization
        ...
        for original, expected in test_cases.items():
            actual = cleaned_df.filter(sellers_df.seller_city == original)\
                             .select("seller_city").collect()
            if actual:
                if expected is None:
                    assert actual[0][0] is None, f"Expected None for {original}, got {actual[0][0]}"
                else:
                    assert actual[0][0] == expected, f"Expected {expected} for {original}, got {actual[0][0]}"
...
def test_state_standardization():
    """Test state name standardization"""
    try:
        sellers_df = create_test_sellers_df()
        
        # Standardize state names
        standardized_df = sellers_df.withColumn(
            "seller_state",
            upper(trim(col("seller_state")))
        )
        
        # Verify all states are uppercase
        non_upper = standardized_df.filter(col("seller_state") != upper(col("seller_state"))).count()
        assert non_upper == 0, "Found non-uppercase state names"
        
        # Test specific cases
        test_cases = ["SP", "MG", "RJ"]
        for state in test_cases:
            count = standardized_df.filter(col("seller_state") == state).count()
            assert count > 0, f"State {state} not found in standardized data"
        
        print("✓ State standardization test passed")
    except Exception as e:
        print(f"❌ State standardization test failed: {str(e)}")
        raise

def test_metropolitan_classification():
    """Test metropolitan area classification"""
    try:
        sellers_df = create_test_sellers_df()
        ...
        cleaned_df = sellers_df.withColumn(
            "seller_city",
            initcap(regexp_replace(lower(trim(col("seller_city"))), "são", "sao"))
        )
        
        # Apply metropolitan classification
        classified_df = cleaned_df.withColumn(
            "metropolitan_area",
            when(col("seller_city").isin("Sao Paulo"), "Sao Paulo Metro")
            .when(col("seller_city").isin("Rio De Janeiro"), "Rio Metro")
            .when(col("seller_city").isin("Bh"), "BH Metro")
            .otherwise("Other")
        )
        
        # Verify classifications
        metro_counts = classified_df.groupBy("metropolitan_area").count()
        
        # Ensure all records have a classification
        null_count = classified_df.filter(col("metropolitan_area").isNull()).count()
        assert null_count == 0, "Found records without metropolitan classification"
        
        # Test specific cases
        sao_paulo_metro = classified_df.filter(col("seller_city") == "Sao Paulo")\
                                     .select("metropolitan_area").collect()[0][0]
        assert sao_paulo_metro == "Sao Paulo Metro", "Incorrect classification for Sao Paulo"
        
        print("✓ Metropolitan classification test passed")
    except Exception as e:
        print(f"❌ Metropolitan classification test failed: {str(e)}")
        raise

# COMMAND ----------
# Run test
def run_all_tests():
    """Run all sellers data cleaning tests"""
    print("Running all sellers data cleaning tests...")
    try:
        test_zip_code_padding()
        test_city_name_cleaning()
        test_state_standardization()
        test_metropolitan_classification()
